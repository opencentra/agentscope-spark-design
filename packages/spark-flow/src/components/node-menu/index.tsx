...
const generateMenuList = ({
  ...
  Object.keys(nodeSchemaMap).forEach((nodeTypeKey) => {
    ...
    if (hasParent && nodeSchemaMap[nodeTypeKey].disableInGroup) {
      const disableInGroup = nodeSchemaMap[nodeTypeKey].disableInGroup;
      if (typeof disableInGroup === 'boolean') {
        return;
      } else if (
        Array.isArray(disableInGroup) &&
        parentNodeType &&
        disableInGroup.includes(parentNodeType)
      ) {
        return;
      }
    }
...
export const NodeMenu = memo((props: IProps) => {
  const NodeMenuItemRenderer = useStore((store) => store.NodeMenuItemRenderer);
  ...
  const parentNode = useMemo(
    () => nodes.find((node) => node.id === props.parentId),
  ...
  const onMenuItemClick = useCallback(
    (data: INodeMenuItemProps['data']) => {
      ...
      const isPrevent = props.onSelect?.(data);
      if (isPrevent !== false) {
...
export const PopoverNodeMenu = memo((props: IPopoverNodeMenuProps) => {
  ...
  return (
    <Popover
      ...
      destroyTooltipOnHide
      getPopupContainer={props.getPopupContainer || ((ele) => ele)}
      content={
        <div
