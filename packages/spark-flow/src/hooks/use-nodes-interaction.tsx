    ...
    async (
      {
        ...
        parentId,
        parentNodeType,
      }: { type: string; parentId?: string; parentNodeType?: string },
      position: { x: number; y: number },
    ) => {
      const nodeInfo = nodeSchemaMap[type];
      ...
      if (parentId) {
        if (
          typeof nodeInfo.disableInGroup === 'boolean' &&
          nodeInfo.disableInGroup
        ) {
          message.warning(
            $i18n.get({
              id: 'spark-flow.hooks.useNodesInteraction.nodeCannotBeAddedToGroup',
              dm: '该节点不能添加到子画布中',
            }),
          );
          return;
        ...
        if (
          parentNodeType &&
          Array.isArray(nodeInfo.disableInGroup) &&
          nodeInfo.disableInGroup.includes(parentNodeType)
        ) {
          message.warning(
            $i18n.get({
              id: 'spark-flow.hooks.useNodesInteraction.nodeCannotBeAddedToGroupWithNodeType',
              dm: '该节点不能添加到该类型子画布中',
            }),
          );
          return;
