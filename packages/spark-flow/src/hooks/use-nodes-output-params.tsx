...
interface ArrayPathInfo {
  fullPath: string; // 该 Array<Object> 元素的完整路径，如 "a.b"
}
...
export const buildOutputParamsTree = ({
  ...
  parentKey = '',
  parentType = '',
  arrayPathStack = [] as ArrayPathInfo[],
  arrayWrap = false,
  flat = false,
}: {
  ...
  parentKey?: string;
  parentType?: string;
  arrayPathStack?: ArrayPathInfo[];
  arrayWrap?: boolean; // 是否将所有属性都包装成数组结构，用于批量节点和循环节点
  flat?: boolean;
}) => {
  const list: IVarItem[] = [];
  ...
  const processItem = (item: INodeDataOutputParamItem) => {
    ...
    if (parentType.startsWith('Array')) {
      itemArrayPathStack.push({ fullPath: itemKey });
    }
    ...
    if (itemArrayPathStack.length > 0) {
      ...
      const firstArrayPath = itemArrayPathStack[0].fullPath;
      const firstArrayParts = firstArrayPath.split('.');

      // baseProperty 是第一个 Array<Object> 之前的路径
      const baseProperty = pathParts
        .slice(0, firstArrayParts.length - 1)
        .join('.');
      ...
      // 从最后一个数组层级开始向前构建
      for (let i = itemArrayPathStack.length - 1; i >= 0; i--) {
        const currentArrayPath = itemArrayPathStack[i].fullPath;
        const currentArrayParts = currentArrayPath.split('.');
        const currentArrayKey = currentArrayParts[currentArrayParts.length - 1];

        if (i === itemArrayPathStack.length - 1) {
          // 最后一层：包含从最后一个数组到当前元素的路径
          const lastArrayParts = currentArrayParts;
          const remainingParts = pathParts.slice(lastArrayParts.length);
          nestedPath =
            remainingParts.length > 0
              ? `${currentArrayKey}.${remainingParts.join('.')}`
              : currentArrayKey;
        } else {
          // 中间层：需要包含到下一个数组之间的路径
          const nextArrayPath = itemArrayPathStack[i + 1].fullPath;
          const nextArrayParts = nextArrayPath.split('.');

          // 计算当前数组和下一个数组之间的中间路径
          const middleParts = nextArrayParts.slice(
            currentArrayParts.length,
            nextArrayParts.length - 1,
          );
          const middlePath =
            middleParts.length > 0 ? `${middleParts.join('.')}.` : '';

          nestedPath = `${currentArrayKey}.${middlePath}[${nestedPath}]`;
        }
      }
      if (arrayWrap) {
        valueFormat = baseProperty
          ? `\${${nodeId}.[${baseProperty}.[${nestedPath}]]}`
          : `\${${nodeId}.[${nestedPath}]}`;
      } else {
        valueFormat = baseProperty
          ? `\${${nodeId}.${baseProperty}.[${nestedPath}]}`
