...
class CoPawAgent(ReActAgent):
    ...
    def __init__(
        ...
        # Tool selector for dynamic tool selection (激进模式)
        self._tool_selector: ToolSelector | None = None  # 延迟初始化，需要model
        # Cache for selected tools during one reply cycle
        self._selected_tools_cache: list[dict] | None = None
    ...
    async def _reasoning(
        ...
        """Override reasoning to use selected tools.

        激进模式：使用已缓存的选中工具（由reply方法选择）
        """
        # 使用选中的工具（由reply方法在选择阶段设置）
        selected_tools = self._selected_tools_cache or []
        ...
        try:
            ...
            logger.debug(
                "=== LLM Request ===\n"
                "Tool choice: %s\n"
                "Tools count: %d (selected) / %d (total)\n"
                "Messages (%d):\n%s",
                tool_choice,
                len(selected_tools),
                len(all_tools),
                len(formatted_msgs),
                self._format_messages_for_log(formatted_msgs),
            )
        except Exception as e:
            logger.debug("Failed to log formatted messages: %s", e)

        # 调用父类方法，但使用选中的工具
        # 通过临时替换 toolkit.get_json_schemas 实现
        original_get_schemas = self.toolkit.get_json_schemas
        self.toolkit.get_json_schemas = lambda: selected_tools  # type: ignore

        try:
            return await super()._reasoning(tool_choice=tool_choice)
        finally:
            # 恢复原始方法
            self.toolkit.get_json_schemas = original_get_schemas  # type: ignore

    def _extract_user_query_from_memory(self) -> str | None:
        """从memory中提取用户query（最后一条用户消息）"""
        try:
    ...
    def _format_messages_for_log(self, messages: list[dict]) -> str:
        ...
        for i, msg in enumerate(messages):
            ...
            if isinstance(content, str) and len(content) > 500:
                ...
                content_preview = []
                for block in content[:5]:  # Limit to first 5 blocks
                    if isinstance(block, dict):
                        ...
                        if block_type == "text":
                            ...
                            content_preview.append(f"[{block_type}]")
                    else:
                        content_preview.append(str(block)[:100])
    ...
    async def reply(
        ...
        # === 动态工具选择（激进模式） ===
        # 在每次reply开始时选择工具
        self._selected_tools_cache = None  # 重置缓存
        ...
        if query and all_tools:
            ...
            # 没有query或没有工具，使用全部工具
            self._selected_tools_cache = all_tools
