import { requestPopSse } from '@agentscope-ai/design';
import { expect, test, vi } from 'vitest';

global.fetch = vi.fn();

test('make POp SSE Request And Get Result', async () => {
  const mockTokenData = {
    code: '200',
    data: {
  ...
  vi.mocked(global.fetch)
    ...
    .mockResolvedValueOnce(
      new Response(`data: ${JSON.stringify(mockData)}\n\n`, {
        status: 200,
        headers: {
